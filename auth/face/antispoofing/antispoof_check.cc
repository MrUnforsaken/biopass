...
std::string resolveDebugPath(const std::string& username, const std::string& configured_path) {
...
bool checkAntiSpoof(const FaceMethodConfig& face_config, const std::string& username,
  ...
  if (ai_enabled) {
    ...
    tasks.push_back(make_task(
        "AI",
        std::async(std::launch::async, [face_config_copy, username_copy, face_copy, config_copy]() {
          return checkAntiSpoofByAIModel(face_config_copy, username_copy, face_copy, config_copy);
        })));
  }
  ...
  if (ir_enabled) {
    ...
    tasks.push_back(make_task(
        "IR", std::async(std::launch::async, [ir_camera_path, detection_model, detection_threshold,
                                              username_copy, debug_enabled, ir_camera_session_ptr,
                                              ir_params]() {
          return checkAntispoofByIRCamera(ir_camera_path, detection_model, detection_threshold,
                                          username_copy, debug_enabled, ir_camera_session_ptr,
                                          ir_params);
        })));
  }
  ...
  if (any_mode) {
    ...
    if (passed > 0) {
      spdlog::debug("FaceAuth: Anti-spoofing passed (any mode, {} passed, {} failed)", passed,
                    failed);
      return true;
    }
    spdlog::error("FaceAuth: Anti-spoofing failed (any mode, {} passed, {} failed)", passed,
                  failed);
    return false;
  }
  ...
  if (failed > 0) {
    spdlog::error("FaceAuth: Anti-spoofing failed (all mode, {} passed, {} failed)", passed,
                  failed);
    return false;
  }
