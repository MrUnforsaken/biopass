...
bool checkAntispoofByIRCamera(const std::string& device_path,
                              ...
                              ICameraCaptureSession* session, const IRCaptureParams& ir_params) {
  ...
  if (!session || !session->isOpen()) {
    local_session =
        openCameraSession(device_path, CameraCaptureFormat::V4L2NV12, ir_params.warmup_frames,
                          ir_params.capture_timeout_ms, ir_params.poll_interval_ms);
    if (!local_session) {
      spdlog::warn("FaceAuth: NV12 IR capture unavailable for '{}'; falling back to GREY",
                   device_path);
      local_session =
          openCameraSession(device_path, CameraCaptureFormat::V4L2Grey, ir_params.warmup_frames,
                            ir_params.capture_timeout_ms, ir_params.poll_interval_ms);
    }
    if (!local_session) {
  ...
  try {
    ...
    // All attempts failed
    spdlog::error(
        "FaceAuth: IR camera check failed, no face detected in {} capture(s) (best: {} face(s))",
