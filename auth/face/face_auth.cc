...
const IRCaptureParams& FaceAuth::irParams() const { return face_config_.advanced.ir_capture; }

bool FaceAuth::isAvailable() const { return checkCameraAvailability(face_config_.camera_device); }
...
AuthResult FaceAuth::authenticate(const std::string& username, const AuthConfig& config,
  ...
  if (!ir_session && face_config_.camera_device.has_value() &&
      *face_config_.camera_device == *face_config_.anti_spoofing.ir_camera) {
    spdlog::debug(
        "FaceAuth: Same device for camera and IR, reusing session and disabling AI anti-spoofing");
